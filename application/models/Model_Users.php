...
class Model_Users extends CI_Model
{
	...
	public function validate_username($username = null)
	{
		if ($username) {
			// die($username);
			$sql = "SELECT * FROM management_users WHERE username = ?";
	...
	public function validate_current_password($password = null, $userId = null)
	{
		if ($password && $userId) {
			...
			$sql = "SELECT * FROM management_users WHERE password = ? AND user_id = ?";
	...
	public function login($username = null, $password = null)
	{
		if ($username && $password) {
			$sql = "SELECT * FROM management_users WHERE username = ? AND password = ?";
	...
	{
		if ($userId) {
			$sql = "SELECT * FROM management_users WHERE user_id = ?";
	...
	public function updateProfile($userId = null)
	{
		if ($userId) {
			...
			$status = $this->db->update('management_users', $update_data);
