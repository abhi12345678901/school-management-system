...
class Model_PrintAdmissionSlip extends CI_Model
{
	...
	*/
	...
	{
		if ($id) {
			$sql = "SELECT * FROM payment WHERE payment_id = ? AND name = 'Admission Fee' ORDER BY payment_id DESC";
			$query = $this->db->query($sql, array($id));
			return $query->row_array();
		}

		$sql = "SELECT * FROM payment WHERE name = 'Admission Fee' ORDER BY payment_id DESC";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
