...
class Model_Accounting extends CI_Model
{
	...
	*/
	public function createIndividual()
	{
		$insert_name = array(
			'name' => 'Monthly Fee',
			'start_date' 	=> $this->input->post('monthlyFeesStartDate'),
			'end_date' 		=> $this->input->post('monthlyFeesEndDate'),
			'total_amount' 	=> $this->input->post('total'),
			'examination_fee'        => $this->input->post('examinationFee'),
			'development_fee'        => $this->input->post('developmentFee'),
			'late_fine'        => $this->input->post('lateFine'),
			'transport_fee'        => $this->input->post('transportFee'),
			'monthly_fee'        => $this->input->post('monthlyFee'),
			'type'			=> 1
		);
	...
	*/
	...
	/*
	*--------------------------------------------------
	* fetches the mothnly fee by the class_id
	*--------------------------------------------------
	*/
	public function monthlyFee($id = null)
	{
		if ($id) {
	...
	*/
	public function removePayment($id = null)
	{
		if ($id) {
			...
			$this->db->where('paymentNameID', $id);
			$payment = $this->db->delete('paymentdaterecord');
	...
	*/
	public function updateStudentPay($paymentId = null)
	{
		if ($paymentId) {
			...
			$payment_name_id = $this->input->post('paymentNameId');
			$updatepayment_data = array(
				...
				'admission_fee' => $this->input->post('admissionFee'),
	...
	*/
	...
	public function totalIncomeByDate($date = null)
	{
		$sql = "SELECT * FROM paymentdaterecord WHERE currentPaidDate = '$date'";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		$totalIncome = 0;
		foreach ($result as $key => $value) {
			$totalIncome += $value['currentPaidAmount'];
		}
	...
	public function fetchIncomeData($date = null)
	{
		if ($date) {
			...
			$sql = "SELECT * FROM paymentdaterecord";
			$query = $this->db->query($sql);
			return $query->result_array();
		}
	}

	public function fetchStudentPayDataByPaymentNameId($id = null)
	{
		if ($id) {
			$sql = "SELECT * FROM payment WHERE payment_name_id = ?";
			$query = $this->db->query($sql, array($id));
